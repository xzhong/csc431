...
public:
	...
	double operator() (int r, int c=0) const{
...
double norm_2(Matrix A) {// The 2-norm of A (only for vectors)
	if(A.cols!=1) 
		throw Exception("OnlyForVectors");
	double s=0;
	for(int i=0; i< A.rows; i++) {
		s += A(i,0)*A(i,0);
	}	
...
Matrix fit(const Matrix &K, int n) {
	Matrix b(K.rows);
	Matrix A(K.rows,n);
	for(int i=0; i<K.rows; i++) {
		for(int j=0; j<n; j++) A(i,j)=pow(K(i,0),j);
		b(i)=K(i,1);
	}
	Matrix x = inv(t(A)*A)*t(A)*b;
	return x;
}

Matrix fit2(const Matrix &K, int n) {
	Matrix b(K.rows,1);
	Matrix A(K.rows,n);
	for(int i=0; i<K.rows; i++) {
		for(int j=0; j<n; j++)
	...
	Matrix a=t(A);
	Matrix L=cholesky(a*A);
	Matrix x=inv(t(L))*inv(L)*a*b;
	return x;
}
...
public:
  ...
  double integrate_adaptative(double a, double b, int n1=3, int n2=5){
	  ...
	  if(abs(I1-I2)<ap) 
		  ...
		  double c = (a+b)/2;
		  double I_left = integrate_adaptative(a,c,n1,n2);
		  double I_right = integrate_adaptative(c,b,n1,n2);
